...
template<class T>
void gl::TransferFunctionGen<T>::generate(const std::string &path, gl::ENDIAN_TYPE endian_type) {
    ...
    std::memset(buffer.get(), 0, buffer_size * sizeof(float));

    for (long long i = 0; i < size; i++) {
        auto vec4 = run((T)(min + i));

        buffer.get()[4 * i] = vec4.x;
        buffer.get()[4 * i + 1] = vec4.y;
        buffer.get()[4 * i + 2] = vec4.z;
        buffer.get()[4 * i + 3] = vec4.w;
    }

    if (endian_type == ENDIAN_TYPE::BIG) {
        gl::internal::convertToBidEndian<float>(buffer.get());
    }
    ...
    out_file.write(reinterpret_cast<char*>(buffer.get()), buffer_size * sizeof(float));
    out_file.close();
}
